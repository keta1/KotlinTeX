...
open class MTMathAtomFactory {
    ...
    fun tableWithEnvironment(
        env: String?,
        cells: MutableList<MutableList<MTMathList>>,
        error: MTParseError
    ): MTMathAtom? {
        ...
        if (matrixEnvs.containsKey(env)) {
            ...
            if (table.numColumns() != 2) {
                error.copyFrom(
                    MTParseError(
                        MTParseErrors.InvalidNumColumns,
                        "$env environment can only have 2 columns"
                    )
                )
                return null
            }
            ...
            if (table.numColumns() != 1) {
                error.copyFrom(
                    MTParseError(
                        MTParseErrors.InvalidNumColumns,
                        "$env environment can only have 1 column"
                    )
                )
                return null
            }
            ...
            if (table.numColumns() != 3) {
                error.copyFrom(
                    MTParseError(
                        MTParseErrors.InvalidNumColumns,
                        "eqnarray environment can only have 3 columns"
                    )
                )
                return null
            }
            ...
            val numCols = table.numColumns()
            // 允许 1 列或 2 列的 cases 环境
            if (numCols != 1 && numCols != 2) {
                error.copyFrom(
                    MTParseError(
                        MTParseErrors.InvalidNumColumns,
                        "cases environment can only have 1 or 2 columns"
                    )
                )
                return null
            }
            ...
            if (numCols == 1) {
                for (i in 0 until table.cells.size) {
                    val row = table.cells[i]
                    if (row.size == 1) {
                        row.add(MTMathList()) // 添加空的第二列
                    }
                }
            }
            table.interRowAdditionalSpacing = 0.0f
