            ...
            when (atom.type) {
                ...
                KMTMathAtomColor -> {
                    ...
                    if (colorAtom.innerList != null) {
                        ...
                        display.localTextColor = MTColor.parseColor(colorAtom.colorString)
...
// index is the index of the element which is getting the sub/super scripts.
    private fun makeScripts(atom: MTMathAtom, display: MTDisplay, index: Int, delta: Float) {
        ...
        require(subScriptList != null || superScriptList != null)
...
// Fractions
    ...
    fun numeratorShiftUp(hasRule: Boolean): Float {
        return if (hasRule) {
            if (this.style == KMTLineStyleDisplay) {
                this.styleFont.mathTable.fractionNumeratorDisplayStyleShiftUp
            } else {
                this.styleFont.mathTable.fractionNumeratorShiftUp
            }
        } else {
            if (this.style == KMTLineStyleDisplay) {
                this.styleFont.mathTable.stackTopDisplayStyleShiftUp
            } else {
                this.styleFont.mathTable.stackTopShiftUp
            }
        }
    }
    ...
    private fun makeFraction(frac: MTFraction): MTDisplay {
        ...
        val barThickness: Float =
            if (frac.hasRule) styleFont.mathTable.fractionRuleThickness else 0.0f
    ...
    private fun addDelimitersToFractionDisplay(
        ...
        require(frac.leftDelimiter != null || frac.rightDelimiter != null)
    ...
    private fun addLimitsToDisplay(
        ...
        if (op.hasLimits && style == KMTLineStyleDisplay) {
            ...
            require(superScript != null || subScript != null) //  At least one of superscript or subscript should have been present.
            val opsDisplay =
                MTLargeOpLimitsDisplay(display, superScript, subScript, delta / 2, 0.0f)
            if (superScript != null) {
    ...
    private fun makeLeftRight(inner: MTInner): MTDisplay {
        require(inner.leftBoundary != null || inner.rightBoundary != null) // Inner should have a boundary to call this function
    ...
    private fun findVariantGlyph(glyph: CGGlyph, maxWidth: Float): CGGlyph {
        ...
        val numVariants = variants.count()
        require(numVariants > 0) //  @"A glyph is always it's own variant, so number of variants should be > 0")
    ...
    private fun getBboxDetailsDescent(bbox: BoundingBox?): Float {
        ...
        return if (bbox == null) {
            ...
            maxOf(0.0f, 0.0f - minOf(bbox.upperRightY, bbox.lowerLeftY))
        }
    }
    ...
    private fun getBboxDetailsAscent(bbox: BoundingBox?): Float {
        return if (bbox == null) {
            ...
            maxOf(0.0f, maxOf(bbox.upperRightY, bbox.lowerLeftY))
        }
    }
